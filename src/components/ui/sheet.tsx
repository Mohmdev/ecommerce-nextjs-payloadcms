...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      'data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0  z-50 bg-black/60 data-[state=closed]:animate-fadeOut data-[state=open]:animate-fadeIn',
      className
    )}
    {...props}
    ref={ref}
  ...
  <SheetPortal>
    ...
    >
      ...
      <SheetPrimitive.Close
        className="absolute right-4 top-6 rounded-md border border-neutral-200 p-1
opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary dark:border-neutral-700"
      >
        <X className="h-4 w-4" />
